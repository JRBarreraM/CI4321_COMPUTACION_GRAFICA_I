...
public class Mario1Controller2DScript : MonoBehaviour {
	...
	void Update () {
		...
		if(!win && GameObject.Find("Flag").transform.position.x < transform.position.x)
		{
			win = true;
	...
	void FixedUpdate () {
		...
		else
		{
			...
			if(win)
			{
				GetComponent<Rigidbody2D>().velocity = new Vector2(0.5f * moveSpeed * Time.deltaTime,GetComponent<Rigidbody2D>().velocity.y);
			}
			else
			{
